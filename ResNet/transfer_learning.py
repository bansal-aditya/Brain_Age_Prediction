...
def train():
    ...
    model = load_model('/export/home/aditya018/code/Resnet/models/BrainAgeResNet(RawT1-Ice)')
    ...
    mc = ModelCheckpoint('BrainAgeResNet(Ice-TransferLearningOnIXI)', verbose=1,
                         mode='min', save_best_only=True)
    early = EarlyStopping(patience=30, verbose=1)

    h = model.fit(
        dataGenerator([get_path(train.Loc.values), train.Scanner.values, train.Gender.values], train.Age.values,
                      batch_size=BATCH_SIZE, meanImg=meanImg, dim=DATA_SHAPE, shuffle=True, augment=True,
                      maxAngle=maxAngle, maxShift=maxShift),
        validation_data=dataGenerator([get_path(val.Loc.values), val.Scanner.values, val.Gender.values],
                                      val.Age.values, batch_size=BATCH_SIZE, meanImg=meanImg, dim=DATA_SHAPE,
