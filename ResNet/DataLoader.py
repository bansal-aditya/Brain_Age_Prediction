...
def getIXIData(imageType):
    path = 'C:/Users/Aditya/Desktop/Research_URECA/Data/IXI/Data/Unziped/'
    paths = []
    ...
    for i in paths:
        idToPath[i.split('/')[-1].split('-')[0][3:]] = idToPath[i.split('/')[-1].split('-')[0][3:]] + [i]

    idFrame = pd.DataFrame.from_dict(idToPathmean, orient='index')
    idFrame['ID'] = idFrame.index
    idFrame.rename(columns={0: 'Loc'}, inplace=True)

    subjectInfoPath = 'C:/Users/Aditya/Desktop/Research_URECA/Data/IXI/Demographic_Data.xlsx'
    subjectInfo = pd.read_excel(subjectInfoPath, sheet_name='IXI_DATA').dropna()

    subjectInfo['ID'] = subjectInfo['ID'].apply(str).apply(lambda x: x.zfill(3))
...
def ORIGINALgetIXIData(imageType):
...
def processing(features, inputShape, meanImg, maxAngle=40, maxShift=10, resizeImg=True, augment=False, training=False):
    X_T1 = loadMR(features)
    #X_T1 = crop_center(X_T1, (121, 145, 121))
...
def crop_center(data, out_sp):
    ...
    start_0 = in_sp[0] // 2 - out_sp[0] // 2
    start_1 = in_sp[1] // 2 - out_sp[1] // 2
    start_2 = in_sp[2] // 2 - out_sp[2] // 2
    data_crop = data[start_0:start_0 + out_sp[0], start_1:start_1 + out_sp[1], start_2:start_2 + out_sp[2]]
    return data_crop
    #
    # in_sp = data.shape
    # nd = np.ndim(data)
